...
// Import the `*PluginMetaPatch` types generated by `#[derive(Patch)]` on each
// plugin's metadata struct — `PluginMetadatas` nests them, so its derive
// references each by name. Also re-exported so tests and handlers can name them.
pub use super::{
    auto_upload::AutoUploadPluginMetaPatch, execute::ExecutePluginMetaPatch,
    game_wrapper::GameWrapperPluginMetaPatch, locale_emulator::LocaleEmulatorPluginMetaPatch,
    translator::TranslatorPluginMetaPatch, voice_speedup::VoiceSpeedupPluginMetaPatch,
    voice_zerointerrupt::VoiceZerointerruptPluginMetaPatch, wine::WinePluginMetaPatch,
};
...
// Nesting on every field: each plugin's metadata can be patched
// individually (e.g. `{ pluginMetadatas: { execute: { enabled: true } } }`)
// without touching the other 7 plugins' state. Combined with the per-field
// `enabled`/`auto_add` patch on each `*PluginMeta`, a toggle sends only
// `{ pluginMetadatas: { <pluginId>: { enabled: <bool> } } }` — minimal wire.
#[patch(no_diff)]
