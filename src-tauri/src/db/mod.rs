...
//   game's `daily_playtime` HashMap over IPC. The TS side (`ListPatchOp`) is generated by ts-rs
//   from struct-patch's `ts` feature — no hand-maintained mirror type.
#[patch(no_diff)]
...
// `PartialEq` on `PluginInstance` and its sub-config structs).
...
// `Option<T>` fields use `nullable`: the patch keeps the double-`Option` type
// so the wire distinguishes missing (no change) from explicit `null` (clear)
// — e.g. clearing `cover_color` when a game's cover changes.
#[patch(no_diff)]
#[patch(skip_serializing_none)]
#[patch(attribute(derive(Debug, Default, Clone, Serialize, Deserialize, TS)))]
#[patch(attribute(ts(export, optional_fields)))]
...
mod tests {
    ...
    #[test]
    fn game_patch_nullable_field_supports_clear() {
        // `#[patch(nullable)]` on `Option<T>` fields: explicit `null` on the
        // wire becomes `Some(None)` and clears the field, while a missing
        // key stays `None` (no change). This is what lets the frontend clear
        // `cover_color` when a game's cover changes.
        let mut game = Game {
            id: 1,
            cover_color: Some("#ff0000".into()),
            excutable_path: Some("/game/a.exe".into()),
            ..Default::default()
        };

        // Missing key -> no change.
        game.apply(GamePatch::default());
        assert_eq!(game.cover_color.as_deref(), Some("#ff0000"));

        // Explicit clear via the double-Option.
        game.apply(GamePatch {
            cover_color: Some(None),
            ..Default::default()
        });
        assert_eq!(game.cover_color, None);
        assert_eq!(game.excutable_path.as_deref(), Some("/game/a.exe"));

        // And the serde round-trip maps `null` to `Some(None)`.
        let patch: GamePatch = serde_json::from_str(r#"{ "excutablePath": null }"#).unwrap();
        game.apply(patch);
        assert_eq!(game.excutable_path, None);
    }
