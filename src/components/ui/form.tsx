 ...
 *
 * Re-exports primitive controls with compact (`size='sm'`)
...
export const FormField: Component<FormFieldProps> = props => (
  <div class={cn('flex flex-col gap-1', props.class)}>
    ...
    <Show when={props.warning}>
      <Hint variant="warning">{props.warning}</Hint>
    </Show>
    <Show when={props.error}>
      <Hint variant="error">{props.error}</Hint>
    </Show>
  </div>
)
...
export interface FormPathInputProps {
  ...
  inputClass?: string
  /** Extra class for the browse `<button>` */
  buttonClass?: string
}
  ...
  return (
    <div class={cn('flex gap-2', props.class)}>
      <Input
        ...
        size="sm"
        value={props.value}
        class={cn('flex-1 min-w-0', props.inputClass)}
      ...
      />
      <Button size="sm" class={props.buttonClass} onClick={handleBrowse}>
        {t('ui.browse')}
      </Button>
    </div>
  )
            ...
            {key => (
              <div class="flex items-center gap-1 bg-white dark:bg-gray-700/50 border border-gray-200 dark:border-transparent px-1 py-0.5 rounded group hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                ...
                <textarea
                  ...
                  class="flex-1 bg-transparent text-gray-800 dark:text-gray-200 text-[11px] px-0.5 rounded border border-transparent hover:border-gray-300 dark:hover:border-gray-600 focus:bg-gray-100 dark:focus:bg-gray-900 focus:border-blue-500 outline-none transition-all min-w-0 resize-none leading-tight"
                />
                <button
                  onClick={() => {
